...
ftag="$1"
commonopt=('--tree' 'Events' '--file' '/nfs/dust/cms/group/exotica-desy/HeavyHiggs/split_trees_UL18_v5/TTTo2L2Nu_TuneCP5_13TeV-powheg-pythia8_em.root' '--variable' 'mtt : 320, 360, 400, 440, 480, 520, 560, 600, 640, 680, 720, 760, 800, 845, 890, 935, 985, 1050, 1140, 1300, 1700' '--variable' 'cHel = negate(b1k * b2k) + negate(b1r * b2r) + negate(b1n * b2n) : 5; -1, 1' '--weight' 'weight' '--bandwidth 0.2' '--bandwidth' '0.2')
...
make -B smoother && ./smoother "${commonopt[@]}" --type weight --mode smooth --systematic QCDscale_MEFac_TT --weight 'wgtu = weight * MEfac_up' --weight 'wgtd = weight * MEfac_down' --output oweight_${ftag}.root && ./smoother "${commonopt[@]}" --type tree --mode smooth --systematic CMS_JER_13TeV --output otree_${ftag}.root && ./smoother "${commonopt[@]}" --mode histogram --output ohistogram_${ftag}.root
